...
public class BookCollection {
	...
	public List<Book> findBookList(Predicate<Book> findMen) {

		return books.stream().filter(findMen).collect(Collectors.toList());
	}

	public Map<Integer, String> findBookMap(Predicate<Book> findMen) {

		return books.stream().filter(findMen)
				.collect(Collectors.toMap((Book b) -> b.getId(), (Book b) -> b.getTitle()));
	}

	public long howManyMatch(Predicate<Book> findMen) {
		return books.stream().filter(findMen).count();
	}
